...
class Person_Detection():
    ...
    def detection(self, im0s): # detect functions
        ...
        im0 = im0s.copy()
        # Process detections
        for i, det in enumerate(pred):  # draw the results
            if len(det):
    ...
    def openimage(self, path):   # open image detection
        try:
            Im = cv2.imread(path)  # read an image by opencv
    ...
    def openvideo(self, path):  # open vedio detection
        # obtain the form of vedio
        try:
        ...
        while success:
            ...
            cv2.waitKey(10)  # delat
            success, frame = videoCapture.read()  # obtain next frame

        videoCapture.release()

    def opencamera(self):   # open camera
        # obtain camero vedio
        try:
...
if __name__ == "__main__":
    ...
    #my.openimage('data/test.jpg') # picture detection
    my.openvideo('data/test.avi') # vedio detection
    #my.opencamera() # camera detection
